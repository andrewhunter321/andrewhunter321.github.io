...
const wheel=$('#funWheel'),wheelLabels=$$('#funWheel span b'),results=['A giant Belgian waffle','A romantic canal photo','A local beer together','A chocolate shop raid','A kiss by the water','An extra boat moment','A ridiculous souvenir','A fancy cocktail'];let wheelRotation=0;$('#spinWheel').addEventListener('click',()=>{if(wheelSpinning)return;wheelSpinning=true;const index=Math.floor(Math.random()*results.length),extra=5+Math.floor(Math.random()*3),target=extra*360+(360-index*45-22.5);wheelRotation+=target;wheel.style.transform=`rotate(${wheelRotation}deg)`;$('#wheelResult').textContent='';sparkle();setTimeout(()=>{wheelLabels.forEach((label,i)=>label.style.transform=`rotate(${-i*45-22.5-wheelRotation}deg)`);wheelSpinning=false;$('#wheelResult').textContent=`The wheel has spoken: ${results[index]}!`;burstConfetti(70);showToast(results[index]+' added to the mission ♥')},5600)});
